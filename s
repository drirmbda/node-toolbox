...
DDEESSCCRRIIPPTTIIOONN
       ssss is used to dump socket statistics. It allows showing information similar to _n_e_t_s_t_a_t.  It can display more TCP and state information than
       other tools.
...
OOPPTTIIOONNSS
       ...
       --zz,, ----ccoonntteexxttss
              As  the  --ZZ  option  but  also shows the socket context. The socket context is taken from the associated inode and is not the actual
       ...
       --AA QQUUEERRYY,, ----qquueerryy==QQUUEERRYY,, ----ssoocckkeett==QQUUEERRYY
              ...
              packet, netlink, unix_dgram, unix_stream, unix_seqpacket, packet_raw, packet_dgram, dccp, sctp, vsock_stream, vsock_dgram,  xdp  Any
...
SSTTAATTEE--FFIILLTTEERR
       SSTTAATTEE--FFIILLTTEERR allows to construct arbitrary set of states to match. Its syntax is sequence of keywords state and exclude followed by identi‐
       fier of state.

       Available identifiers are:

              All standard TCP states: eessttaabblliisshheedd, ssyynn--sseenntt, ssyynn--rreeccvv, ffiinn--wwaaiitt--11, ffiinn--wwaaiitt--22, ttiimmee--wwaaiitt, cclloosseedd, cclloossee--wwaaiitt, llaasstt--aacckk, lliisstteenniinngg
              and cclloossiinngg..
...
EEXXPPRREESSSSIIOONN
       EEXXPPRREESSSSIIOONN  allows  filtering based on specific criteria.  EEXXPPRREESSSSIIOONN consists of a series of predicates combined by boolean operators. The
       possible operators in increasing order of precedence are oorr (or | or ||), aanndd (or & or &&), and nnoott (or !). If no operator is between  con‐
       secutive predicates, an implicit aanndd operator is assumed. Subexpressions can be grouped with "(" and ")".
       ...
       ffwwmmaarrkk [[==||!!==]] MMAASSKK
              Matches based on the fwmark value for the connection. This can either be a specific mark value or a mark value followed by a "/" and
              a  bitmask of which bits to use in the comparison. For example "fwmark = 0x01/0x03" would match if the two least significant bits of
              the fwmark were 0x01.
...
HHOOSSTT SSYYNNTTAAXX
       ...
       FAMILY must be one of the families supported by the -f option. If not given it defaults to the family given with the -f option, and if that
       is  also missing, will assume either inet or inet6. Note that all host conditions in the expression should either all be the same family or
       be only inet and inet6. If there is some other mixture of families, the results will probably be unexpected.

       The form of ADDRESS and PORT depends on the family used. "*" can be used as a wildcard for either the address or port. The details for each
       family are as follows:

       uunniixx   ADDRESS  is a glob pattern (see ffnnmmaattcchh(3)) that will be matched case-insensitively against the unix socket's address. Both path and
              abstract names are supported. Unix addresses do not support a port, and "*" cannot be used as a wildcard.

       lliinnkk   ADDRESS is the case-insensitive name of an Ethernet protocol to match. PORT is either a device name or a device index  for  the  de‐
              sired link device, as seen in the output of ip link.

       nneettlliinnkk
              ADDRESS is a descriptor of the netlink family. Possible values come from /etc/iproute2/nl_protos. PORT is the port id of the socket,
              which is usually the same as the owning process id. The value "kernel" can be used to represent the kernel (port id of 0).
